...
class TestWebserverUnit < MiniTest::Test

   def test_init 
    server = Webserver.new(7080,"./test_files")
  ...
  def test_get_blank_url_redirects_to_index
    inval = "GET HTTP/1.1"
    expected = "index.html"
    server = Webserver.new(7083,"./test_files")
  ...
  def test_get_just_slash_url_redirects_to_index
    inval = "GET / HTTP/1.1"
    expected = "index.html"
    server = Webserver.new(7084,"./test_files")
    assert_equal(expected, server.get_filename(inval))
  end
  ...
  def test_get_filename
    ...
    expected = "test"
    server = Webserver.new(7085,"./test_files")
    assert_equal(expected, server.get_filename(inval))
  end
  ...
  def test_get_url_with_html_and_slashes
    ...
    server = Webserver.new(7086,"./test_files")
    assert_equal(expected, server.get_filename(inval))
  end    

  def test_get_web_formatted_non_file
    inval = "GET /notafile HTTP/1.1"
    server = Webserver.new(7087,"./test_files")
