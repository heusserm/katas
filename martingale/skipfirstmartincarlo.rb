puts "\n--------------------------------------------------------------------------"
puts "skipfirstmartincarlo.rb"
puts "(C) 2021 Matthew Heusser Matt@xndev.com"
puts "----------------------------------------------------------------------------"
puts "A Monte Carol simulator of the Martingale betting technique. Uses the heusser"
puts "skip first method, which I just made up.\n"
...
def run_hand(current_bet, bankroll,largest_bet_so_far, iteration_num,last_hand) 
  ...
  if (rand <= $odds)
    ...
    bankroll=bankroll-current_bet;
    #ONLY double if PREVIOUS hand was a loser and we lost again
    if last_hand == "loser "
  ...
  return current_bet,bankroll,largest_bet_so_far,last_hand;
end
...
for sessions in 0..num_sessions
  for i in 0..num_bets_per_session
    current_bet,bankroll, max_bet, last_hand = run_hand(current_bet, bankroll, max_bet,i,last_hand) 
  end
